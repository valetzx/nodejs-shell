...
app.use("/ray", createProxyMiddleware({ target: "http://localhost:2098", changeOrigin: true, ws: true, onError: (err, req, res) => { console.error("[Proxy] error:", err.message); if (!res.headersSent) res.writeHead(500, { "Content-Type": "text/plain" }); res.end("failed"); } }));
